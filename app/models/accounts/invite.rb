...
class Accounts::Invite < ApplicationRecord
  ...
  def user_already_in_organization
    ...
    return unless organization.users.find_by(id: recipient)

    errors.add(:recipient, 'already exists in the organization!')
  end

  def user_already_invited
    return unless Accounts::Invite.exists?(email: email, accepted_at: nil, organization: organization)
  ...
  def accept_only_once
    return unless accepted_at.present?

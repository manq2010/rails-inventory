class StagedRolloutComponent < ViewComponent::Base
  ...
  def actions
    ...
    if controllable_rollout?
      if created?
        actions << { form_url: increase_release_path, confirm: START_RELEASE_CONFIRM, type: :blue,
                     name: 'Start Rollout' }
      end
      if started?
        actions << { form_url: increase_release_path, confirm: RELEASE_CONFIRM, type: :blue,
    ...
    if automatic_rollout?
      if paused?
        actions << { form_url: resume_release_path, confirm: RESUME_RELEASE_CONFIRM, type: :blue,
                     name: 'Resume Phased Release' }
      end
      if started? && last_rollout_percentage
        actions << { form_url: pause_release_path, confirm: PAUSE_RELEASE_CONFIRM, type: :amber,
    ...
    if last_rollout_percentage
      if started? || paused?
        actions << { form_url: halt_release_path, confirm: HALT_CONFIRM, type: :red,
                     name: 'Halt Release' }
      end
      if started?
        actions << { form_url: full_release_path, confirm: FULLY_RELEASE_CONFIRM, type: :blue,
                     name: 'Release to 100%' }
      end
    end
